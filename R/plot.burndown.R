plot.burndown <- function(x, col=NULL, draw.plan=TRUE, draw.regression=FALSE, draw.lastupdate=FALSE, t.stop="", debug=FALSE, ...)
{
	...
	# legend
	ll <- if (length(x$task$description) < 10) 1.1 else 1.0
